...
//规则名：添加用户规则时不允许添加的ip 
$.validator.addMethod("ipNotAllow127", function(value) {  
   return value != "127.0.0.1";  
}, '不允许设置ip为127.0.0.1');
$.validator.addMethod("ipNotAllow0", function(value) {  
	   return value != "0.0.0.0";  
	}, '不允许设置ip为0.0.0.0');
//规则名：os_admin,value检测对像的值  
$.validator.addMethod("os_admin", function(value) {  
   return value != "os_admin";  
}, '名称不能为os_admin'); 
//规则名：添加用户规则时先选择用户  
$.validator.addMethod("chooseUserFirst", function(value) {  
   return value != "请选择用户";  
}, '请选择用户');
//规则名：root,value检测对像的值  
	$.validator.addMethod("root", function(value) {  
	   return value != "root";  
	}, '名称不能为root');  
...
//字符验证+','
 $.validator.addMethod("EstringCheckAddComma", function(value, element) {
return this.optional(element) || /^([0-9a-zA-Z_,])*[0-9a-zA-Z_]+$/.test(value);// /^([0-9a-zA-Z_,])*[0-9a-zA-Z_]+$/
}, "只能包括字母、数字和下划线, 也可以用逗号分隔的列表来创建数据库，如：database1, database2");
